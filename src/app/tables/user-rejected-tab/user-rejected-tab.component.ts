...
@Component({
  selector: 'app-user-rejected-tab',
  templateUrl: './user-rejected-tab.component.html',
  styleUrls: ['./user-rejected-tab.component.css']
})
export class UserRejectedTabComponent implements AfterViewInit,OnInit {
  displayedColumns = ['username', 'leaveReason', 'fromDate', 'toDate', 'createdAt', 'leaveType', 'noOfDays', 'status', 'adminRemark', 'lop', 'actions'];
  private rejectedLeaves:any  
  empId
  id
...
getRejectLeaves(){
  this.leaveService.getRejectedLeavesByUser(this.id).subscribe(data => {
