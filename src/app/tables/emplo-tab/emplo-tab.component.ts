...
@Component({
  selector: 'app-emplo-tab',
  templateUrl: './emplo-tab.component.html',
  styleUrls: ['./emplo-tab.component.css']
})
export class EmploTabComponent implements OnInit,AfterViewInit{
  ...
  dataSourceLe = new MatTableDataSource<any>();
...
getAll(){
  this.leaveService.getleavesByUser(this.id).subscribe(
      ...
      this.dataSourceAll.sort = this.sort;
		  ...
		  // this.allLeavesCountVar = data.len;
		},
		err =>{
...
getApproved(){
  this.leaveService.getApprovedLeavesByUser(this.id).subscribe(
      ...
      this.dataSourceA.sort = this.sort;
		  ...
		  console.log(this.dataSourceA.data)
...
getPending(){
  this.leaveService.getPendingLeavesByUser(this.id).subscribe(
      ...
      this.dataSourceP.sort = this.sort;
		  ...
		  console.log(this.dataSourceP.data)
		},
		err =>{
	...
	getLeavesinrange(fromDate,toDate){
		let x = this.leaveService.leavesInRange(fromDate,toDate).subscribe(
			data =>{
				...
				this.dataSourceLe.data = this.leavesInRangeVar;
	  ...
	  viewLeaves(rowData) {
	   ...
	   // this.dialog.open(LeaveDetailsCardComponent, dialogConfig);   
		...
		ngAfterViewInit(): void {
		  this.dataSourceAll.sort = this.sort;
		  this.dataSourceAll.paginator = this.paginator;
		  console.log(this.dataSourceAll.data)
		}
		reLoad(){
